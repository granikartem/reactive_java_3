...
public class ConcurrentUserTasksCompletionCollector implements Collector<Task, Map<User, Map<Task, Boolean>>, Map<User, Map<Task, Boolean>>> {
    private final boolean useDelay;

    public ConcurrentUserTasksCompletionCollector(boolean useDelay) {
        this.useDelay = useDelay;
    }

    public static ConcurrentUserTasksCompletionCollector toTaskStatusStatsMap(boolean useDelay) {
        return new ConcurrentUserTasksCompletionCollector(useDelay);
    }
    ...
    public BiConsumer<Map<User, Map<Task, Boolean>>, Task> accumulator() {
        return (map, task) -> {
            ...
            if (map.containsKey(user)) {
                ...
                var resultMap = new ConcurrentHashMap<Task, Boolean>();
    ...
    public Set<Characteristics> characteristics() {
        return Set.of(Characteristics.UNORDERED, IDENTITY_FINISH, Characteristics.CONCURRENT);
    }
}
